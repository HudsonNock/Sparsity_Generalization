    ...
    for num_maps, lmbda in test_pairs:
        for j in range(2):
            if j == 0:
                ...
                num_seeds = 300
            percentage_complete = np.empty((13, num_seeds))
            for i in range(13):
                if j == 0:
                ...
                step_num = int(100*i)
                if lmbda != -1:
                    load_path = f"Sparsity_Checkpoints_maps\\chkpt_{lmbda}_{num_maps}_{step_num}.pth"
                else:
                    load_path = f"Sparsity_Checkpoints_maps\\chkpt_SAC_{num_maps}_{step_num}.pth"
                policy = build_agents_and_buffer(device, load_path, (lmbda != -1))
                epsilon = 0.05
                max_steps = 300 #500
                ...
                print(f"Starting data collection... {lmbda}")
            ...
            if lmbda != -1:
                if j == 0:
                    np.save(f"results\\chkpt_{lmbda}_{num_maps}_Train.npy", percentage_complete)
                else:
                    np.save(f"results\\chkpt_{lmbda}_{num_maps}_Test.npy", percentage_complete)
            else:
                if j == 0:
                    np.save(f"results\\chkpt_SAC_{num_maps}_Train.npy", percentage_complete)
                else:
                    np.save(f"results\\chkpt_SAC_{num_maps}_Test.npy", percentage_complete)
